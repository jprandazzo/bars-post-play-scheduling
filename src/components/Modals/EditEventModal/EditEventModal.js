...
export const EditEventModal = ({
  ...
  const filteredDaysOfWeek = uniqueDaysOfWeek.filter(day => normalizeString(day).includes(normalizeString(editedEvent.sportDayOfWeek)))
  const filteredLocations = uniqueLocations.filter(location =>
    normalizeString(location).includes(normalizeString(editedEvent.location))
  );
  const filteredWtnbOrCoed = uniqueWtnbOrCoed.filter(option =>
    normalizeString(option).includes(normalizeString(editedEvent.wtnbOrCoed))
  );

  const formatDateForInput = (date) => date instanceof Date ? date?.toISOString().split('T')[0] : '';
  ...
  return (
    ...
    <Modal isOpen={isOpen} onRequestClose={onClose} style={customStyles}>
      ...
      >
        ...
        <button type="button"className="cancel-btn" onClick={handleDiscardChanges}>Discard Changes</button>
      </div>
